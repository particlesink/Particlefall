...
How to obtain LIGGGHTS(R)-PUBLIC
-----------------------------------------------
...
.. note::
   ...
   Users of a LIGGGHTS-PREMIUM version will need to replace the
...
`here <https://github.com/CFDEMproject/LIGGGHTS-PUBLIC/archive/master.zip>`_
...
Making LIGGGHTS(R)-PUBLIC
----------------------------------------

This section has the following sub-sections:

* :ref:`Read this first <start_2_1>`
* :ref:`Steps to build a LIGGGHTS(R)-PUBLIC executable <start_2_2>`
* :ref:`Advanced building using custom makefiles <start_2_2_expert>`
* :ref:`Building using cmake <start_2_2_cmake>`
* :ref:`Common errors that can occur when making LIGGGHTS(R)-PUBLIC <start_2_3>`
...
please post a question to the `LIGGGHTS(R)-PUBLIC forum <http://www.cfdem.com/forum>`_.
...
building description :ref:`here <start_2_2_expert>` or use :ref:`cmake <start_2_2_cmake>` build system.
...
lucky enough it will actually produce an executable called *lmp\_auto* in your
*src/* folder. If this is the case you are done and can move on to `Running LIGGGHTS(R)-PUBLIC <start_6>`_
...
parameters of the variable called \*USE\_MPI\*. A short description of the
...
user and machine specific paths. Say that \*USE\_VTK\* is set to "ON" and your
...
In this case you would want to change \*VTK\_INC\_USR\* and \*VTK\_LIB\_USR\* so
that they become
...
.. note::
   ...
   If you set a \*\_USR\* variable, such as \*VTK\_INC\_USR\* and its
   corresponding non-\*\_USR\* variable (\*VTK\_INC\* in our example) is set. Then
...
.. note::
   ...
   If you change any options in the first part of your
...
finished you will obtain a executable name \*lmp\_auto\* and you can continue
:ref:`here <start_6>`.

.. note::

   Should the compilation not work due to some deficiency of the
   ...
   `forums <http://www.cfdem.com/forum>`_ so that we can improve it for future
   use.
...
create a symbolic link. So after the compilation is complete and your lmp\_auto
...
.. parsed-literal::
   ...
   su -c "ln -s lmp_auto /usr/local/bin"

.. note::

   You need administrator access to run the two commands listed above.
...
This has the same effect as changing the \*USE\_DEBUG\* variable inside your
...
\*lmp\_auto-debug\* or \*lmp\_auto-fulldebug\* for "ON" and "FULL", respectively.

.. note::

   If you want to clean this particular compilation \*make clean-auto\*
...
but \*MAKE/Makefile.user\_myString\*. If it does not exist it will clone
\*MAKE/Makefile.user\* (instead of a default template). The resulting
executable will be called \*lmp\_auto\_myString\* (instead of \*lmp\_auto\*).
...
In this case you can create a file called \*style\_contact\_model\_user.whitelist\*
...
During every compilation the three whitelist files \*style\_contact\_model\_user.whitelist\* ,
\*style\_contact\_model\_autoExamples.whitelist\* and \*style\_contact\_model.whitelist\* are
...
If you get no errors and an executable like lmp\_mpi or lmp\_serial is produced,
you're done; it's your lucky day.

Note that by default only a few of LIGGGHTS(R)-PUBLIC optional packages are
installed.  To build LIGGGHTS(R)-PUBLIC with optional packages, see :ref:`this section <start_3>` below.
...
.. note::
   ...
   Do not attempt to copy and modify Makefile.auto as this is a rather
...
ACML, and T3E.  For backward compatability, using -DFFT\_FFTW will use
...
which LIGGGHTS(R)-PUBLIC uses when writing out JPEG or PNG files via the :doc:`dump image <dump_image>` command.  These can be left blank if you do not
...
Note that by default only a few of LIGGGHTS(R)-PUBLIC optional packages are
installed.  To build LIGGGHTS(R)-PUBLIC with optional packages, see :ref:`this section <start_3>` below, before proceeding to Step 9.
...
.. note::
   ...
   The ccmake program can help to fine-tune the available cmake flags.
...
- DISABLE\_ALL:BOOL=OFF
- ENABLE\_ALL:BOOL=OFF
- ENABLE\_BIGBIG:BOOL=OFF
- ENABLE\_FFMPEG:BOOL=OFF
- ENABLE\_GZIP:BOOL=OFF
- ENABLE\_JPEG:BOOL=OFF
- ENABLE\_LONGLONG\_TO\_LONG:BOOL=OFF
- ENABLE\_MODEL\_COHESION\_EASO\_CAPILLARY\_VISCOUS:BOOL=OFF
- ENABLE\_MODEL\_COHESION\_OFF:BOOL=OFF
- ENABLE\_MODEL\_COHESION\_SJKR:BOOL=ON
- ENABLE\_MODEL\_COHESION\_SJKR2:BOOL=ON
- ENABLE\_MODEL\_COHESION\_WASHINO\_CAPILLARY\_VISCOUS:BOOL=OFF
- ENABLE\_MODEL\_EDINBURGH:BOOL=OFF
- ENABLE\_MODEL\_EDINBURGH\_STIFFNESS:BOOL=OFF
- ENABLE\_MODEL\_HERTZ:BOOL=ON
- ENABLE\_MODEL\_HERTZ\_STIFFNESS:BOOL=OFF
- ENABLE\_MODEL\_HOOKE:BOOL=OFF
- ENABLE\_MODEL\_HOOKE\_HYSTERESIS:BOOL=OFF
- ENABLE\_MODEL\_HOOKE\_STIFFNESS:BOOL=OFF
- ENABLE\_MODEL\_LUDING:BOOL=OFF
- ENABLE\_MODEL\_ROLLING\_CDT:BOOL=OFF
- ENABLE\_MODEL\_ROLLING\_EPSD:BOOL=OFF
- ENABLE\_MODEL\_ROLLING\_EPSD2:BOOL=ON
- ENABLE\_MODEL\_ROLLING\_EPSD3:BOOL=OFF
- ENABLE\_MODEL\_ROLLING\_LUDING:BOOL=OFF
- ENABLE\_MODEL\_ROLLING\_OFF:BOOL=OFF
- ENABLE\_MODEL\_SURFACE\_DEFAULT:BOOL=ON
- ENABLE\_MODEL\_SURFACE\_MULTICONTACT:BOOL=OFF
- ENABLE\_MODEL\_SURFACE\_SUPERQUADRIC:BOOL=OFF
- ENABLE\_MODEL\_TANGENTIAL\_HISTORY:BOOL=ON
- ENABLE\_MODEL\_TANGENTIAL\_LUDING:BOOL=OFF
- ENABLE\_MODEL\_TANGENTIAL\_NO\_HISTORY:BOOL=ON
- ENABLE\_MODEL\_TANGENTIAL\_OFF:BOOL=OFF
- ENABLE\_MODEL\_THORNTON\_NING:BOOL=OFF
- ENABLE\_MPI:BOOL=ON
- ENABLE\_PNG:BOOL=OFF
- ENABLE\_SMALLBIG:BOOL=ON
- ENABLE\_SMALLSMALL:BOOL=OFF
- ENABLE\_SQ:BOOL=OFF
- ENABLE\_VTK:BOOL=ON
...
.. warning::
   ...
   If an error occurs when building LIGGGHTS(R)-PUBLIC, the compiler
...
(3) Changing the LIGGGHTS(R)-PUBLIC size limits via -DLAMMPS\_SMALLBIG or
...
`CMake <www.cmake.org>`_ and launch the associated GUI. Point the *Where is your
...
the *VTK\_DIR* option to the folder in which you built the VTK library. Once this
...
The other way to do this is install and use `cygwin <www.cygwin.org>`_ to build
...
Making LIGGGHTS(R)-PUBLIC with optional packages
---------------------------------------------------------------
...
* :ref:`Additional Makefile settings for extra libraries <start_3_4>`
...
.. warning::
   ...
   You should NOT include/exclude packages and build
...
Building LIGGGHTS(R)-PUBLIC via the Make.py script
-----------------------------------------------------------------
...
Building LIGGGHTS(R)-PUBLIC as a library
-------------------------------------------------------
...
LIGGGHTS(R)-PUBLIC to other codes.  See :doc:`this section <Section_python>` for
...
src/MAKE/Makefile.foo.  The build will create the file liblammps\_foo.a which
...
will create the file liblammps\_foo.so which another application can link to
...
wish to add the LIGGGHTS(R)-PUBLIC src directory to LD\_LIBRARY\_PATH, so that the
...
When used from a C++ program, all of LIGGGHTS(R)-PUBLIC is wrapped in a LAMMPS\_NS
...
using LIGGGHTS(R)-PUBLIC as a library.  See :ref:`Section\_howto 19 <howto_19>` of the manual for a description of the
...
Running LIGGGHTS(R)-PUBLIC
-----------------------------------------

By default, LIGGGHTS(R)-PUBLIC runs by reading commands from stdin; e.g. lmp\_auto
...
* At the command prompt, type "lmp\_win\_no-mpi -in in.lj", replacing in.lj
  with the name of your LIGGGHTS(R)-PUBLIC input script.
...
* You'll need to use the mpiexec.exe and smpd.exe files from the MPICH2 package. Put them in
  same directory (or path) as the LIGGGHTS(R)-PUBLIC Windows executable.
* Get a command prompt by going to Start->Run... ,
  then typing "cmd".
...
* Then type something like this: "mpiexec -np 4 -localonly lmp\_win\_mpi -in in.lj",
  replacing in.lj with the name of your LIGGGHTS(R)-PUBLIC input script.
...
For example, lmp\_auto might be launched as follows:
...
LIGGGHTS(R)-PUBLIC reads its input script from stdin - e.g. lmp\_auto < in.run.
...
LIGGGHTS(R)-PUBLIC screen output
-----------------------------------------------
