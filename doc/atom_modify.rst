...
Modify properties of the atom style selected.

The *map* keyword determines how atom ID lookup is done for molecular
problems.  Lookups are performed by bond (angle, etc) routines to 
find the local atom index associated with a global atom ID.
...
your input script.  The group is not used until a simulation
is run.
...
cache performance and thus speed-up a simulation, as discussed
in a paper by :ref:`(Meloni) <Meloni>`.  Its efficiency depends on the problem
