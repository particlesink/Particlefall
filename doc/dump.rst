...
* style = *atom* or *atom/vtk* or *xyz* or *image* or *local* or *custom* or *mesh/stl* or *mesh/vtk* or *mesh/vtm* or *decomposition/vtk*
  ...
  .. parsed-literal::
       ...
       *mesh/vtk* args =  zero or more keyword/ value pairs followed by one or more dump-identifiers followed by one or more mesh ids
           ...
           *output* values = 'face' or 'interpolate' or 'original'
           ...
           mesh-ids = any ID of a :doc:`fix mesh/surface <fix_mesh_surface>`
       *mesh/vtm* args =  one or two keyword/ value(s) pairs
           keywords = *meshes* (required) or *mesh_properties*
           *meshes* value(s) = mesh-ids
           mesh-ids = either keyword 'all' or a list of IDs of :doc:`fix mesh/surface <fix_mesh_surface>`
...
.. parsed-literal::
   ...
   dump 5 all custom/vtk 0.1 sec dump_time.\*.vtk id type x y z
...
As a prototype, the dump cadence can also be specified directly in
physical time by adding a trailing *sec* or *s* suffix.  This is
intended to replace common input-script patterns that manually convert
time intervals to timestep counts.
...
:doc:`fix mesh <fix_mesh>` commands into the specified file.
...
geometries defined via :doc:`fix mesh <fix_mesh>` commands to a series of VTK
files. The *mesh/vtk* style allows the file endings .vtk, .vtp and .pvtp. The
first two write the data only on one processor, the latter writes several files
in parallel. The *mesh/vtm* style currently only allows the .vtm file ending
...
optional keyword *mesh\_properties* that needs to be followed by the dump
...
:doc:`fix mesh/surface <fix_mesh_surface>` commands, you can specify which meshes to dump.
...
For all vkt outputs if *dump\_modify binary* is used, the dump file (or files, if
...
faster. Additionally, *dump\_modify compressor* can be used to specify the
...
file for molecular systems. *id\_multisphere* is the ID of the multisphere
