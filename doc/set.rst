...
* keyword = *type* or *type/fraction* or *mol* or *x* or *y* or *z* or           *vx* or *vy* or *vz* or           *omegax* or *omegay* or *omegaz* or           *charge* or *quat* or *quat\_direct* or           *quat/random* or *diameter* or *shape* or           *length* or *tri* or *theta* or *angmom* or           *mass* or *density* or *volume* or *image* or           *bond* or *add* or *until* or           *property/atom* or *blockiness*
  ...
  .. parsed-literal::
       ...
       *omegax*\ ,\ *omegay*\ ,\ *omegaz* value = atom rotational velocity (rad / time units)
         value can be an atom-style variable (see below)
       *inertiax*\ ,\ *inertiay*\ ,\ *inertiaz* value = atom principal moments of inertia for superquadric particles (distance\^2 mass units)
         value can be an atom-style variable (see below)  
...
particles must be ellipsoids as defined by the :doc:`atom\_style ellipsoid <atom_style>` command, triangles as defined by the
:doc:`atom\_style tri <atom_style>` command or superquadric as defined by the
...
the :doc:`atom\_style ellipsoid <atom_style>` command, triangles as
defined by the :doc:`atom\_style tri <atom_style>` command or superquadric as
defined by the :doc:`atom\_style superquadric <atom_style>` command.  Random
...
with keyword *quat*\ , for ellipsoidal and superquadric particles, the 3 shape values
must be non-zero for each particle set by this command.  This keyword
does not allow use of an atom-style variable.

Keyword *quat\_direct* simply creates a quaternion from it's values instead of 
calculting these from a unit rotation vector and rotation angle. These keyword 
is an alternative to the keyword *quat*\ .
...
particles must be ellipsoids or superquadric as defined by the :doc:`atom\_style ellipsoid <atom_style>` or :doc:`atom\_style superquadric <atom_style>` command.  The *Sx*\ , *Sy*\ , *Sz* settings 
...
particles must be ellipsoids as defined by the :doc:`atom\_style ellipsoid <atom_style>` command, triangles as defined by the
:doc:`atom\_style tri <atom_style>` command or superquadrics as defined 
by the :doc:`atom\_style superquadric <atom_style>` command. The angular 
...
Keywords *bond*  set the bond type  of all bonds of selected atoms to
...
nbondtype  was set by the *bond types* field in the
header of the data file read by the :doc:`read\_data <read_data>`
...
Keywords *shape* and *blockiness* define superquadric shape of a particle.
...
Keywords *quat\_direct* and *blockiness* require :doc:`atom\_style <atom_style>` *superquadric*
